...
model_nd = model_dir + 'attention-2020-09-30_11_47_54-model_best.pth'  # AttenResnet4, c=16, sigmoid, attention residual
models = [model_nd]
...
def main():
    ...
    for model_i in models:
        ...
        checkpoint = torch.load(model_i, map_location=map_location)
        model.load_state_dict(checkpoint['state_dict'])
        model_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
        print('model params is', model_params)

        retrieve_weight(args, model, device, eval_loader, args.plot_dir, rnn)
