"""
Filename: main.py
Objective: Appy GMM classifier to ivector (400 dim)
Author: Jian Yang
Created: Jan. 23, 2021
...
def train(train_txt, logger):
    ...
    """
    X_genuine = np.empty((0, 400), float)
    X_spoof = np.empty((0, 400), float)
    ...
    for line in label_train:
        ...
        if items[0].startswith('0'):
            # spoof
            X_spoof = np.concatenate((X_spoof, ivector.reshape(1, -1)), axis=0)
        elif items[0].startswith('1'):
            # genuine
            X_genuine = np.concatenate((X_genuine, ivector.reshape(1, -1)), axis=0)
        else:
...
def main():
    parser = argparse.ArgumentParser(description='ivector-GMM for spoof detection')
    parser.add_argument('--train-txt', required=True,
                        help='train ivector in txt')
    ...
    # set up loggers
    run_name = "gmm-ivector" + time.strftime("-%Y-%m-%d_%H_%M_%S") + args.suffix
